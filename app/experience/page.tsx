...
import { ABOUT_STORY, DOCUMENTARY, DREAMY_FINE_ART, EDITORIAL, FILM } from "@/lib/images";
import { CORE, PRIMARY_REGIONS, SITE, TIERS, quoteFor } from "@/lib/site";

export const metadata: Metadata = {
  title: "The Experience — What It Is Like to Be Photographed",
  description:
    "From the first email to the printed album: how a wedding day is planned, how it is photographed, and what comes back and when.",
  alternates: { canonical: "/experience" },
  openGraph: {
    title: "The Experience | Arman Arai",
    description: "How a wedding day is planned, photographed and handed back.",
    url: `${SITE.url}/experience`,
    images: [{ url: EDITORIAL[2].src, alt: EDITORIAL[2].alt }],
  },
};
...
export default function Experience() {
  ...
  return (
    <>
      ...
      {/* ── HERO ── */}
      <section className="relative" style={{ height: "min(72vh, 640px)", minHeight: 420 }}>
        <Image
          src={EDITORIAL[2].src}
          alt={EDITORIAL[2].alt}
          fill
        ...
        >
          ...
          <h1 className="font-serif font-light text-cream leading-[1.02]" style={{ fontSize: "clamp(2.5rem,5.5vw,4.8rem)" }}>
            What it is like to be <em className="italic text-rose">photographed</em>
          </h1>
          <p className="text-blush text-[1rem] leading-relaxed max-w-2xl mx-auto mt-6 font-light">
            Six chapters, in the order you will live them. Nothing here is a surprise on the day,
      ...
      {CHAPTERS.map((c, i) => {
        ...
        return (
          <section
            key={c.numeral}
            className={`py-16 md:py-24 ${i % 2 === 0 ? "bg-ivory" : "bg-parchment"}`}
          >
            <div className="page-w page-px grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
              <div className={imageRight ? "lg:order-1" : "lg:order-2"}>
                <p className="text-[0.62rem] tracking-[0.32em] uppercase text-rose mb-4">{c.kicker}</p>
                <h2 className="font-serif font-light text-cream mb-6 leading-tight" style={{ fontSize: "clamp(1.9rem,3.2vw,3rem)" }}>
                  <span className="text-rose mr-3">{c.numeral}</span>
                  {c.title}
                </h2>
                {c.body.map((para) => (
                  <p key={para.slice(0, 40)} className="text-slate text-[0.95rem] leading-[1.85] mb-5 last:mb-0">
                    {para}
                  </p>
                ))}
              </div>
              <div
                className={`relative w-full ${imageRight ? "lg:order-2" : "lg:order-1"}`}
